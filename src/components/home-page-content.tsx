...
import HackathonsSection from "./section/hackathons-section";
...
export default function HomePageContent() {
  const t = useTranslations(); // locale comes from context
          ...
          <div className="gap-2 gap-y-6 flex flex-col md:flex-row justify-between">
            <div className="gap-2 flex flex-col order-2 md:order-1">
              <BlurFadeText
                ...
                yOffset={8}
                text={t("hero.hi", { name: DATA.name.split(" ")[1] })}
              />
              <BlurFadeText
        ...
        <div className="flex min-h-0 flex-col gap-y-6">
          ...
          <div className="flex flex-col gap-8">
            {DATA.education.map((education, index) => {
              const eduKey = index === 0 ? 'giaic' : 'nasra';
              const translatedDegree = t(`education.items.${eduKey}.degree`);
              return (
                <BlurFade
      ...
      <section id="favourite-tools">
        <div className="flex min-h-0 flex-col gap-y-4">
          <BlurFade delay={BLUR_FADE_DELAY * 10.5}>
            <h2 className="text-xl font-bold">{t("favouriteTools.title")}</h2>
          </BlurFade>
          <div className="flex flex-wrap gap-2">
          ...
          <div className="flex min-h-0 flex-col gap-y-4">
            ...
            <div className="flex flex-col gap-2">
              {DATA.certifications.map((cert, id) => (
                <BlurFade key={cert.name} delay={BLUR_FADE_DELAY * 15 + id * 0.05}>
                  <div className="border bg-background border-border ring-2 ring-border/20 rounded-xl p-4 flex flex-col gap-1">
                    <div className="font-semibold text-sm">{cert.name}</div>
                    <div className="text-xs text-muted-foreground">{cert.issuer}{cert.date && ` - ${cert.date}`}</div>
                  </div>
                </BlurFade>
              ))}
            </div>
