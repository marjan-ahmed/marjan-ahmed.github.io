...
export default function WorkSection() {
  ...
  const workKeys = ['naxasware', 'reintechs'] as const;
  return (
    <Accordion type="single" collapsible className="w-full grid gap-6">
      {DATA.work.map((work: typeof DATA.work[number], index) => {
        const key = workKeys[index];
        const translatedTitle = t(`work.items.${key}.title`);
        const translatedDesc = t(`work.items.${key}.description`);
        return (
          <AccordionItem
            key={work.company}
            value={work.company}
